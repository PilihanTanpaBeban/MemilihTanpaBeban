...
import {
  alignmentTypes,
  indonesiaProvinces,
} from "../../app/components/calon/const";
...
import {
  getAllDataV2,
  getSearchResultV2,
} from "../../app/components/calon/model/APIService";
...
import { ScrollAfterProvClick } from "../../app/util/ViewUtil";

const Calon: React.FC = () => {
  // select search
  const [selectedProvince, setSelectedProvince] = useState<string | null>("ID");
  const [searchProvince, setSearchProvince] = useState<string | null>(null);
  const [province, setProvince] = useState<string | null>(null);
  const searchTarget = useRef<HTMLDivElement>(null);
  const [modalVisibleDPR, { open: openDPR, close: closeDPR }] =
    useDisclosure(false);
  const [modalVisibleGubernur, { open: openGubernur, close: closeGubernur }] =
    useDisclosure(false);
  ...
  const [alignmentType, setAlignmentType] = useState<string | null>(null);
  const [selectedAlignmentType, setSelectedAlignmentType] = useState<
    string | null
  >("0");
  ...
  const fetchData = async (page: number) => {
    ...
    try {
      ...
      pejabatTypeParam == "gubernur"
        ? setLimitGubernur(response.totalData)
        : setLimitDPR(response.totalData);
  ...
  const handleScroll = () => {
    if (
      ...
      if (
        (dataDPR.length > 1 && dataDPR.length < limitDPR) ||
        (dataGubernur.length > 1 && dataGubernur.length < limitGubernur)
      )
  ...
  const handleProvinceClick = (Province_id: string) => {
    ...
    ScrollAfterProvClick(searchTarget);
  };
  ...
  }, [searchProvince, province, alignmentType]);
    ...
    <Box pb={rem(150)} bg={"#F7FAFF"}>
      <Box bg={lightPurple}>
        <Container size="xl" py={rem(56)}>
          <Flex direction={"column"} align={"center"} justify={"center"}>
            ...
            <Map
              mapWidth={
                mini ? 300 : mobile ? 700 : tablet ? 800 : laptop ? 1000 : 1200
              }
      ...
      <Container size={"xl"} ref={searchTarget}>
        <Flex
          h={!mobile ? rem(44) : "100%"}
          mt={rem(56)}
          mb={!mobile ? rem(100) : rem(50)}
          gap={"xl"}
          direction={mobile ? "column" : "row"}
          align="center"
          justify={mobile || tablet ? "center" : "start"}
        >
          ...
          <Select
            ...
            data={indonesiaProvinces.map((category) => ({
              value: category.id,
          ...
          />{" "}
          <label htmlFor="alignment-select">Argumen</label>
          <Select
            classNames={{ input: classes.input }}
            value={selectedAlignmentType || alignmentType}
            placeholder="Argumen"
            data={alignmentTypes.map((type) => ({
              value: type.id,
              label: type.title,
            }))}
            maxDropdownHeight={200}
            clearable
            onChange={setSelectedAlignmentType}
          />
          <Button
            onClick={() =>
              handleRequestSearch(selectedProvince, selectedAlignmentType)
            }
            h={"100%"}
            px={rem(40)}
            py={rem(11)}
        ...
        {(dataDPR.length > 0 || dataGubernur.length > 0) && (
          <Title
            ta={"center"}
            mb={rem(50)}
            c={primaryColor}
            fz={mobile ? rem(18) : rem(30)}
          >
            Klik Untuk Deskripsi Lebih Detail
          </Title>
        )}
        <Box
          pos="relative"
          style={{ padding: mobile || tablet ? "0 12px" : "0" }}
        >
          <LoadingOverlay
            visible={loading}
            zIndex={1000}
      ...
      <Modal
        centered
        size={mobile || tablet ? "100%" : "70%"}
        opened={modalVisibleDPR}
        onClose={closeDPR}
      ...
      <Modal
        centered
        size={"auto"}
        opened={modalVisibleGubernur}
        onClose={closeGubernur}
        withCloseButton={false}
      >
        {selectedGubernur !== undefined && (
          <ModalDetailGubernur data={selectedGubernur} />
        )}
      </Modal>
    </Box>
  );
