...
const CalonPejabatBox: React.FC<CalonPejabatBoxProps> = ({ pejabat_id, opini, name, partai, province, onBoxClick }) => {
    ...
    return (
        <div onClick={handleBoxClick} style={{ backgroundColor: white, borderRadius: rem(5), cursor: 'pointer', maxWidth: rem(283), height: 'auto', boxShadow: '0 4px 4px rgba(0, 0, 0, 0.25)', overflow: 'hidden' }}>
            ...
            <Flex bg="white" align="center" justify="center" p={rem(10)} direction="column">
                <Text mb={rem(13)} style={{ fontSize: rem(16) }} fw="600" ta={'center'}>
                    {name} ({partai})
                </Text>
                <Text ta={'center'} mb={rem(7)}>Dapil {province}</Text>
            </Flex>
        </div>
    );
};
