  ...
  return (
    <Box h={mobile ? "100svh" : loading ? "50vh" : "100%"} pos={"relative"}>
      ...
      {!loading && !detailPejabat && (
        <Text ta={"center"} style={{ fontSize: rem(16) }}>
        ...
        <Box p={mobile || tablet ? rem(20) : rem(40)} pos={"relative"}>
          {detailPejabat.map((pejabat, index) => (
            <>
              <Flex
                gap={"md"}
                direction={mobile || tablet || laptop? "column" : "row"}
                key={pejabat.Pejabat_id}
              >
                <Flex gap={"33px"} direction={mobile ? "column" : "row"}>
                  ...
                  <Flex direction={"column"} gap={rem(10)}>
                    <Flex
                      ...
                      style={{
                        borderRadius: rem(9),
                        boxShadow: `0 0 ${rem(7)} rgba(0, 0, 0, 0.15)`,
                      }}
                    >
                      <div
                        style={{
                          ...
                          height: "18px",
                          borderRadius: "50%",
                          backgroundColor: checkColor(pejabat.Alignment_Name),
                        }}
                      />
                      <Text
                    ...
                    <Flex direction={"column"}>
                      <Title
                        style={{ fontSize: rem(30) }}
                        w={!mobile ? rem(350) : "100%"}
                        mb={rem(5)}
                        mt={rem(10)}
                      >
                        {pejabat.Pejabat_Name}
                      </Title>
                      <Text>Calon {pejabat.Pejabat_Type}</Text>
                    </Flex>
                    ...
                    <Flex gap={"sm"} direction={"column"} maw={mobile?undefined:rem(300)}>
                      <Text style={{ fontSize: rem(14) }}>
                        Partai
                      </Text>
                ...
                <Flex
                  className={classes.textQuote}
                  gap={"md"}
                  mt={mobile || tablet ? rem(25) : rem(60)}
                  direction={"column"}
                >
                  <Title style={{ fontSize: rem(24) }} c={primaryColor}>
                    Quote:
                  </Title>
                  {pejabat.Quote_Desc == null && (
                    <Text style={{ fontSize: rem(16) }}>
                      Belum ada statement di media massa terkait isu
                      pengendalian tembakau
                    </Text>
                  )}
                  <Text ta={"justify"}>
                    {pejabat.Quote_Desc != null &&
                      renderTextWithHtml(pejabat.Quote_Desc)}
                  </Text>
                </Flex>
              </Flex>
              {index != detailPejabat.length - 1 ? (
                <Divider my={rem(25)} />
              ) : null}
            </>
          ))}
